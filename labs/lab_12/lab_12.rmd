...
```{r}
write_csv(md_only,str_c("data/maryland_zips_",today(),".csv"))
write_csv(non_md,str_c("data/non_maryland_zips_",today(),".csv"))
```
...
```{r}
pop_vax <- all_md_pop %>% 
  left_join(all_md_vax, by = 'zip_code') %>% 
...
ggplot() +
  ...
  geom_sf(data=pop_vax, aes(color=pct_vax))
```
