...
```
## 2 - Filter dates & group
```{r}
wv_ppp_apr <- wv_ppp %>% 
  filter(
    date_approved >= '2020-04-01' &
...
wv_ppp_apr %>% 
  ggplot() +
  geom_line(aes(x=date_approved, y=count_loans)) +
  geom_point(aes(x=date_approved, y=count_loans)) +
  geom_text(aes(x=date_approved, y=count_loans, label=count_loans, hjust=1.25)) +
  scale_x_date(date_breaks = '1 day', date_labels = '%b %d') +
  theme(
    axis.text.x = element_text(angle = 45,  hjust=1)
  ) +
  labs(
    title="West Virginia PPP Loans by Date Approved",
    x="\nDate",
    y="Loans Approved\n",
...
## 2 - Census Data
```{r}
acs_2019 <- load_variables(2019, "acs1", cache=TRUE)
```

## 2 - Pop Dataset & Fix Dakotas
```{r}
pop_2019 <- get_acs(geography="State", variable="B01001_001", year=2019) %>% 
...
## 2 - Join & per 100,000 column
```{r}
ppp_pop <- all_ppp %>% 
  left_join(pop_2019, by="state") %>% 
  mutate(loans_per_hundo=(total_ppp_loans/estimate)*100000) %>% 
  arrange(desc(loans_per_hundo))
```
...
ppp_pop %>% head(10) %>% 
  ggplot +
  geom_bar(aes(reorder(x=state,loans_per_hundo), weight=loans_per_hundo), fill="blue", position = position_dodge(width=0.5)) +
  coord_flip() +
  labs(
    title = "States by PPP Loans per Hundred Thousand Residents\n",
    x = "State\n",
    y = "Total Loans",
    caption = "Source: Small Business Administration & US Census"
  )
```
**A2.** 
In descending order: South Dakota, North Dakota, Nebraska, Georgia, Iowa, Illinois, Louisiana, Florida, Mississippi, and Montana.
...
## 3 - Organize Data: Loans < 25,000, NA = 0
```{r}
wv_small <- wv_ppp %>% 
  filter(
    amount < 25000
...
**A3.**  
It looks like the vast majority of businesses had their loans forgiven entirely, or had none of their loan forgiven. Of the businesses who had a partial amount forgiven, most of them had close to the full amount forgiven.