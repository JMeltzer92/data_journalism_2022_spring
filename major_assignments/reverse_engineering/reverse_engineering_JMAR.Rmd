...
* **Analysis summary**: We used ggplot on a few cities to make sure there was indication of an increase in total homicides across the country, which there is. This is where it gets tricky: the language the Post uses is vague. "The greatest raw number increase in killings" since when? Our data shows both Baltimore and Chicago total murders soaring in the 2014-2016 period, but it seems that the journalists chose somewhat vague language instead painting an exact picture, which may have been very wordy.
...
homicide_by_city %>% 
  ...
  ggplot() + 
  geom_line(aes(x=year,y=total_homicides))
```
```{r}
homicide_by_city %>% 
  filter(city=="Chicago") %>% 
