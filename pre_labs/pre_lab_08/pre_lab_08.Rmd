...
**Answer**
Somerset and Caroline Counties, only the former is outside the margin of error.
```{r}
md10 %>%
...
**Answer**
21202, 21740 & 21201. The only one that is definitely under $50,000 when accounting for margin of error is 21201.
```{r}
md_zipcodes <- get_acs(geography="zcta", variables = "B19013_001", state='MD')
md_zips_with_acs <- top_zips %>% left_join(md_zipcodes, by=c('zip5'='GEOID')) %>% top_n(20, total)
