...
**Answer**
150,000+ rows of tweets from public officials and candidates that contain the words "COVID" and/or "coronavirus."
```{r}
...
**Answer**
Removes duplicate words - case sensitive
```{r}
...
**Answer**
pulls out every unique word (and number) from all the tweets in our dataframe - all 5.4 million of them :0
```{r}
...
**Answer**
Yes, these words are good words.
```{r}
unique_words %>%
...
**Answer**
Most of the same words are in the top 10 in both years; but we can't see any meaningful thoughts.
```{r}
unique_words_2020 %>%
  anti_join(stop_words) %>%
  group_by(word) %>%
...
**Task** Run the following code and describe the results.
**Answer**
two word phrases instead of standalone single words
```{r}
covid_tweets %>%
  filter(created < '2021-01-01') %>%
...
**Task** Run the following code and describe the results.
**Answer**
more or less the same two-word phrases as above, with the notable exception of vaccines making a more prominent appearance towards the top
```{r}
covid_tweets %>%
  filter(created >= '2021-01-01') %>%
  unnest_tokens(bigram, content, token = "ngrams", n = 2) %>%
...
**Task** Run the following code and describe the results.
**Answer**
Ds tweet far more about public health and social distancing and things like that, while Rs tweet less about covid in general, and do not bring up the minutiae like Ds did.
```{r}
covid_tweets %>%
...
**Task** Run the following code and describe the results.
**Answer**
Lots of D candidates tweeted about president Trump and nursing homes, while lots of R candidates tweeted about... illegal immigrants and being COVID positive.
```{r}
covid_tweets %>%
...
**Answer**
I REALLY doubt that all 7000+ mentions of Trump catalogued were positive in 2020, but most of the other entries seem pretty positive.
```{r}
