...
**Answer**
Daily number of vaccinations by county in MD
```{r}
...
**Answer**
It looks like some counties are missing days, or have days with no data. 
...
**Answer**
It's a bunch of entries that have no county assigned
...
**Answer**
New vaccinations have been decreasing since the beginning of March.
```{r}
...
**Answer**
A huge spike early on, then decreasing over time with smaller spikes coinciding with upticks in cases and new variants.
```{r fig.width=9}
ggplot() +
...
**Answer**
The cumulative cases by county in MD.
```{r}
...
**Task** Run the following code and describe what the resulting chart shows
**Answer**
A logarithmic scale for the chart above.
```{r}
...
**Answer**
Maryland counties color coded by number of cumulative cases.
```{r}
ggplot() +
