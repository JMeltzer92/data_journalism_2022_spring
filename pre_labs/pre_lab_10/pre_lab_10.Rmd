...
https://datawrapper.dwcdn.net/DDXOj/1/
...
It's grouping all the loans by the county in which they were issued and creating a chart with the county names, and the total loans issued within each.
```{r}
...
**Answer** Describe what the code below is doing and the output.
The same as above, but removing all counties with less than 6 loans.
```{r}
md_counties <- maryland_ppp %>%
    group_by(project_county_name) %>%
    summarise(count=n()) %>%
...
A five-digit code that combines the state and county codes
```{r}
...
The two frames joined together: we have loans per county as well as the fips code needed for the maps.
```{r}
...
**Answer** Describe the changes from the previous map
Has a title, labeled legend, subtitle, and improved captions
...
https://datawrapper.dwcdn.net/t5jce/1/
