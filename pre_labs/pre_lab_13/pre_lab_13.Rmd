...
**Answer**
A rough correlation between pct voting for trump and % of business receiving ppp loans
```{r}
...
**Task** Run the following code. Describe the plot. What do you see?
**Answer**
a plot with a less convincing relationship between trump votes and ppp dollars per employee
```{r}
...
**Answer**
A new table showing the r value of pct_trump vs the other two variables.
```{r}
reveal_data %>%
...
**Answer**
A messy code block, but it shows that the data is probably not an abberation or coincidence.
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer**
Similar code block to above - not as likely to be a slam dunk this time.
```{r}
...
**Answer**
a familiar looking chart that shows two averages for states that went fr clinton and trump
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer**
another ugly code block, showing that there's not a strong correlation between a state's vote tally and the amount of money received per employee.
```{r}
...
**Task** Run the following code. Describe the output. What do you see?
**Answer**
A much stronger correlation between the number of businesses that received ppp money and the way the state voted in 2016.
```{r}
