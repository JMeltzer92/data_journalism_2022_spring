...
**Answer**:
135073, which is the same number as the amount we saw in Task 8.
```{r}
ppp_maryland_loans %>%
...
**Answer**
I think the mininmum amount is more suspicious. It makes sense that some companies would get $10000000, since that's the maximum amount allowed under the PPP law... but who is getting six bucks?...
```{r}
...
**Answer**
Columm headers! The id has a 1 in front of it, NAME is in all caps while none of the other headers are, and street address is missing the all-importnat underscore. 
```{r}
arnold_md_loans
```
...
**Answer**
characters cannot be added up, because the total_amount column has the wrong datatype
```{r}
# cleaning function
...
**Answer**
Stuart Jones LLC has two entries.
```{r}
cleaned_arnold_md_loans %>%
  get_dupes()
```
...
**Answer**
None... but I need some clarification on the bit with TRUE at the end
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
