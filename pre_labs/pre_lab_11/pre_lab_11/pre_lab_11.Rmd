...
**Answer**
A glimpse of the dataframe. There are 33 columns, including whether or not there's a trauma center at each hopsital, how many beds they have, etc.
```{r}
...
**Answer**
Another glimpse, it's different than tidyverse though?
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Another glimpse of a bunch of geo_id data
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
A plot of where the hospitals are on a grid that isn't outlined
```{r}
md_hospitals %>%
  ggplot() + 
  geom_sf() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
a new dataframe of just maryland counties
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
A map of Maryland with the bay filled in. Cursed map
```{r}
md_counties %>%
  ggplot() + 
  geom_sf() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
the same map with dots for each hospital
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
A map with color coded dots for how many beds each hospital has.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
A table showing the total hopsital beds per county.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Another one of those not aesthetically pleasing geo data tables
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
a matchable table with geodata stuff
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
a geo table with the NA values removed in total beds, replaced with 0
```{r}
md_beds_per_100k <- md_county_population %>%
  left_join(md_beds_by_county) %>%
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
a map with MD counties color coded by hospital beds available per hundred thousand
```{r}
ggplot() + 
...
**Answer**
the counties around DC seem to be underserved. The Eastern Shore/bay is all over the place, with some counties massively overserved and some that have 0 beds at all.
```{r}
ggplot() + 
  geom_sf(data=md_beds_per_100k, aes(fill=beds_per_100k)) + 
  theme_minimal() +
