...
An outline of the various and sundry industries that encompass virtually all of the businesses in the US. It's a good overview of NAICS codes and what they mean.
...
**Answer**
The HTML that produces the webpage that users see.
...
**Answer**
A list with two entries, one for "head" and one for "body" with all of the code for each HTML section on one line... zoinks
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**
more html code showing how the sausage of the table is made.
...
**Answer**
seven different tibbles! so exciting. so poorly formatted. wauw!
```{r}
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**
A list of the six tables scraped from the NAICS website, but each arrow shows a string of code instead of a table.
...
**Answer**
Look! It's a bird! It's a plane! It's a familiar looking table! (serious: It's a table with the data from the NAICS website. something about the dates different NAICS systems were used. very cool, not boring or irrelevant at all.)
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**
*waves jedi hand* these are the numbers you're looking for. It's the NAICS code breakdown we actually wanted.
...
**Answer**
Hey look, it's a poorly formatted table that has all the NAICS codes we want!
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
BLS data about the Utilities sector of the US economy.
...
**Answer**
The HTML code that created the table titled "iag22emp1"
```{r}
...
**Answer**
An unwieldy table, but at least it's readable and not a string of code.
```{r}
...
**Answer**
A cleaned table that only shows the number of workers employed in the Utilities sector and none of the other stuff from the last step.
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
Information about the mining sector. The layout is almost identical to that we saw for the Utilities sector.
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
The same as above, but for the Construction sector.
...
**Answer**
Pretty much the same thing as we typed in, but with a [1] in front of each category name
```{r}
...
**Answer** Nothing, but a list of sectors appears on the environment tab under "values"
```{r}
...
**Answer** the same list as in task 14.
```{r}
...
**Answer**
an error because sector_list doesn't exist. makes sense.
```{r, error=TRUE}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer**
We get the same list we did in Task 14, but with fewer (?) keystrokes.
```{r}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer** There's an error because x and sector_name are mismatched.
```{r, error=TRUE}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Answer**
It printed every number from 1 to 10.
```{r}
# For loop that includes instructions that refer to a variable that doesn't exist.
for (number in 1:10) {
  print(number)
...
**Answer**
The NAICS table we generated in chapter 16.
```{r}
...
**Answer**
The number of rows in our NAICS table.
```{r}
nrow(naics_industry)
```
...
**Answer**
a list of numbers from 1 to 20, which happens to be the number of rows in our NAICS table. coincidence? I think not!
```{r}
# For loop that includes instructions that refer to a variable that doesn't exist.
...
**Answer**
There's the Utilities employment table we scraped and cleaned earlier.
```{r}
...
**Answer**
a new table, but it has the URL of every NAICS page. so cool!
```{r}
...
**Answer**
Same as above, but with the xpath id for each table we want to use
```{r}
...
**Answer**
Same as above but without the row for Public Administration.
```{r}
...
**Answer**
A table with each entry separated from our above table.
```{r}
...
**Answer**
A custom printed string with all the different urls and xpaths that we want.
```{r}
...
**Answer**
all 19 of the tables for each of the NAICS codes
```{r}
...
**Answer**
a bunch of single-row employment tables for each NAICS industry code
```{r}
...
**Answer**
The same as above, but with the NAICS code and industyr name apended to the end of each table.
```{r}
...
**Task** Run the following for loop to clean up tables. Briefly describe the output that appears below the codeblock.
**Answer**
Same as above, but cleaned and only showing employment numbers for Nov 2021.
```{r}
...
... no it's the table we've been working on for 50+ tasks. Hooray!
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
Employment info for the mining sector.
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** Employment info, or lack thereof, for the agriculture sector.
...
**Answer**
The nice pretty table we created earlier but with an NA value in the top row, aka Agriculture.
```{r}
